...
class Layout extends Component {
    ...
    componentDidMount() {
        if(this.props.pageInfoData.loggedInUserDetails)
            this.props.socket.emit('chatJoin', {id:this.props.pageInfoData.loggedInUserDetails.user_id})
        if(this.props.translateT){
            this.props.translateT(this.props)
        }
        ...
        userChannel.onmessage = data => {
            if (data.payload.type === "LOGIN") {
                ...
                if (this.props.pageData.loggedInUserDetails) {
                    this.props.socket.emit('chatLeave', {id:this.props.pageInfoData.loggedInUserDetails.user_id})
                    Router.push("/")
    ...
    render() {
        ...
        const generalInfo = this.props.pageInfoData.pageInfo ? this.props.pageInfoData.pageInfo : {}
        ...
        if (this.props.pageInfoData.appSettings["fixed_header"] == 1 && this.state.width > 992) {
            if (!this.props.pageInfoData.removeFixedMenu)
                fixedHeader = " fixed-header"
            else
                fixedHeader = " fixed-header nofixed-header"
            if (this.props.hideSmallMenu || this.props.pageInfoData.hideSmallMenu) {
                disableMarginLeftClass = " marginLeft0"
            }
        }
        if (this.props.pageInfoData.appSettings["fixed_header"] == 0 && this.state.width > 992) {
        ...
        if (this.state.width > 992)
            fixedMenu = (this.props.hideSmallMenu || this.props.pageInfoData.hideSmallMenu ? " top-menu" : " sidemenu") + disableMarginLeftClass + (this.props.menuOpen ? "" : " sidemenu-opened")
        
        CDN_URL_FOR_STATIC_RESOURCES = CDN_URL_FOR_STATIC_RESOURCES + this.props.pageInfoData.subFolder
        ...
        return (
            <React.Fragment>
                    ...
                    <Head>
                            ...
                            : null}
                        {
                            this.props.pageInfoData.appSettings["pwa_app_name"] ?
                                <React.Fragment>
                                    <link rel='manifest' href={`${this.props.pageInfoData.subFolder}manifest.json`} />
                ...
                {
                   this.props.chatMessages || (this.props.artistView && this.props.pageInfoData.photos && this.props.pageInfoData.photos.results.length > 0) ||
                        (this.props.pageInfoData.movie && this.props.pageInfoData.movie.images && this.props.pageInfoData.movie.images.length > 0) ?
                        <Head>
                    ...
                    videoURL != "" ?
                        <Head>
                            <script type="application/ld+json" dangerouslySetInnerHTML={{
                                ...
                                "description": "${this.props.pageInfoData.video.description}",
                    ...
                    !isIE && !this.props.embedVideo ?
                        !this.props.liveStreaming ?
                            <WithErrorHandler {...this.props}>
                                <div className={`main-content${this.props.pageInfoData.themeType == 2 ? " adv-theme" : ""}${fixedMenu}${this.props.pageInfoData.loggedInUserDetails ? " user-logged-in" + fixedHeader : fixedHeader}${this.state.width > 992 ? "" : " mobile-layout-cnt"}${this.props.pageInfoData.slideshow || (this.props.pageInfoData.videos && this.props.pageInfoData.videos.featured) ? " slideshow-enable" : ""}${this.props.pageInfoData.showHomeButtom == 1 || this.props.pageInfoData.userProfilePage == 1 ? " user-subscription-page" : ""}${` ${(generalInfo.type ? generalInfo.type : "no-data").replace(/_/g, '-')}-cs-page`}`}>
                                    {
                                    ...
                                    <div className={`content-wrap${this.props.menuOpen && !this.props.hideSmallMenu && this.state.width > 992 ? " ml100" : this.props.menuOpen && this.props.hideSmallMenu && this.state.width > 992 ? " ml0" : ""}${this.state.width > 992 ? "" : " mobile-layout"}${this.props.pageInfoData.showHomeButtom == 1 || this.props.pageInfoData.userProfilePage ? " user-subscription-cnt" : ""}`}>
                                        {
                                            this.props.pageInfoData.appSettings['below_header'] ?
                                                <AdsIndex className={`${this.props.pageInfoData.hideSmallMenu ? "nopad-ads " : ""}header_advertisement`} paddingTop="90px" ads={this.props.pageInfoData.appSettings['below_header']} />
                                                : null
                                        }
                                        {this.props.children}
                                    </div>
