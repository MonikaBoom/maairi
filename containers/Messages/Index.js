...
class Messages extends React.Component {
    ...
    constructor(props){
        ...
        this.state = {
            showEmoji:false,
            textValue:"",
            messages:props.pageInfoData.messages,
            pagging:props.pageInfoData.pagging,
            page:2,
            messageId:props.pageInfoData.messageId,
            openMessage:props.pageInfoData.openMessage,
            chatMessages:props.pageInfoData.chatMessages
        }
    ...
    componentDidMount() {
        this.props.socket.on('chatDelete', data => {
            ...
            if (itemIndex > -1) {
                ...
                if(this.state.messageId == id){
                    if(messages.length > 0){
                        messageID = messages[0].message_id
                    }
                }
                this.setState({localUpdate:true, messages: messages,messageId:messageID })
            }
        })
    ...
    pushTab = (id) => {
        ...
        this.setState({messageId:id,localUpdate:true,openMessage:this.state.messages[itemIndex]})
        Router.push(`/messanger?messageId=${id}`, `/messages/${id}`,{ shallow: true })
    }
    changeTextValue = (e) => {
                                        ...
                                        <ul className="list-unstyled chat-list chat-user-list">
                                            {
                                                this.state.messages.length > 0 ?
                                                    this.state.messages.map(message => {
                                                        let type = "m"
                                                        if(message.resource_id != this.props.pageInfoData.loggedInUserDetails.user_id){
                                                            type = "r"
                                                        }
                                                        return (
                                                            <li className={`${parseInt(message.is_read) == 0 && message.last_user_id != this.props.pageInfoData.loggedInUserDetails.user_id ? "unread" : ""}${this.state.messageId == message.message_id ? " active" : ""}`} key={message.message_id}> 
                                                                <a href={`/messages/${message.message_id}`} onClick={(e) => {
                                                                    ...
                                                                    <div className="d-flex">
                                                                        <div className="chat-user-img online align-self-center me-3 ms-0">
                                                                            <img src={this.props.pageInfoData.imageSuffix+message[type+"avtar"]}
                                                                        ...
                                                                        <div className="flex-1 overflow-hidden">
                                                                            ...
                                                                            <p className="chat-user-message text-truncate mb-0">
                                                                                {
                                                                                    message.last_user_id == this.props.pageInfoData.loggedInUserDetails.user_id && message.message.trim() != "" ? 
                                                                                        this.props.t("You: ")
                                                                                    :null
                                                                                }
                                                                                {
                                                                        ...
                                                                        <div className="font-size-11">
                                                                            ...
                                                                            {
                                                                                message.last_user_id == this.props.pageInfoData.loggedInUserDetails.user_id && parseInt(message.seen) == 1 ? 
                    ...
                    <div className="d-lg-flex">
                        {
                            ...
                            <a href="#" className="back-messages" onClick={(e) => {
                                e.preventDefault();
                                this.setState({messageId:null,localUpdate:true});
                            }}>
                        ...
                        <Chat {...this.props} newChat={this.newChat} messageId={this.state.messageId} message={this.state.openMessage} chats={this.state.chatMessages ? this.state.chatMessages : []} />
                        : <div className="no-content">
