...
class Chat extends React.Component {
    constructor(props){
        ...
        this.state = {
            ...
            pagging:false,
            page:1,
            messageId:props.pageInfoData.messageId,
            loading:true,
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        ...
        }else if (nextProps.messageId != prevState.messageId){
            return {
                showEmoji:false,
                textValue:"",
                chats:[],
    ...
    componentDidMount() {
        ...
        this.props.socket.on('chatMessageDelete', data => {
            let id = data.id
            if(parseInt(data.message_id) == parseInt(this.state.messageId)){
                const itemIndex = this.getItemIndex(id)
                if (itemIndex > -1) {
                    const chats = [...this.state.chats]
                    chats.splice(itemIndex, 1);
                    if(this.props.newChat){
                        this.props.newChat(chats,this.state.messageId)
                    }
                    this.setState({localUpdate:true, chats: chats })
                }
            }else{
                if(this.props.newChat){
                    this.props.newChat("delete",this.state.messageId,id);
                }
            }
        })
        this.props.socket.on('chatMessageCreate', data => {
            ...
            if(parseInt(chat.message_id) == parseInt(this.state.messageId)){
                this.props.socket.emit('readChat', {id:this.props.pageInfoData.loggedInUserDetails.user_id,message_id:this.state.messageId})
                if(parseInt(chat.chat_user_id) != parseInt(this.props.pageInfoData.loggedInUserDetails.user_id)){
                    chat.is_read = 1;
                    chat.seen = 1;
                }
    ...
    loadMoreData = (type) => {
        ...
        let url = `/messages/${this.state.messageId}`;
    ...
    openImage = (url,e) => {
        ...
        let items = []
        items.push({
            src:  (isS3 ? this.props.pageInfoData.imageSuffix : "") + url,
            title: "",
            description: "",
            type: 'image'
        });
    ...
    submitForm = () => {
        if(this.state.selectedFile || this.state.textValue){
            ...
            formData.append("message_id",this.state.messageId)
    ...
    render() {
        let type = "m"
        if(this.state.message && this.state.message.resource_id != this.props.pageInfoData.loggedInUserDetails.user_id){
            type = "r"
        }
                        ...
                        <div className="col-sm-12 col-12">
                            <div className="d-flex align-items-center flex-wrap">
                                <div className="me-3 ms-0 ml5">
                                    <Link href="/member" customParam={`memberId=${this.state.message[`${type}username`]}`} as={`/${this.state.message[`${type}username`]}`}>
                                        <a>
                                            <img src={this.props.pageInfoData.imageSuffix+this.state.message[`${type}avtar`]} className="rounded-circle avatar-xs" alt={this.state.message[`${type}displayname`]} />
                                        </a>
                                    </Link>
                                </div>
                                ...
                                <div className="pull-right chatTopIconActn">
                                    ...
                                    </a>
                                    {
                                        ...
                                        <React.Fragment>                                            
                                            <a href="#" onClick={(e) => {
                                                e.preventDefault();
                                                this.props.minimizeChat(this.state.messageId);
                                            }}>
                                                {
                                                    this.state.minimize ? 
                                            ...
                                            <a href="#" onClick={(e) => {
                                                e.preventDefault();
                                                this.props.closeChat(this.state.messageId);
                                            }}>
                            ...
                            }
                            {
                                this.state.chats.length > 0 ? 
                                    this.state.chats.map(chat => {
                                        return (
                                            <li key={chat.messages_text_id} className={`${parseInt(chat.chat_user_id) == this.props.pageInfoData.loggedInUserDetails.user_id ? "right" : ""}`}>
                                                <div className="conversation-list">
                                                    <div className="chat-avatar">
                                                        <img src={this.props.pageInfoData.imageSuffix+chat.avtar}
                                                    ...
                                                    <div className="user-chat-content">
                                                        <div className="ctext-wrap">
                                                                                ...
                                                                                <li
                                                                                    ...
                                                                                    <div>
                                                                                        <a className="popup-img d-inline-block m-1"
                                                                                            href="#" onClick={(e) => {this.openImage(this.props.pageInfoData.imageSuffix+chat.image,e)}}>
                                                                                            <img src={this.props.pageInfoData.imageSuffix+chat.image}
                                                            ...
                                                            {
                                                                chat.chat_user_id == this.props.pageInfoData.loggedInUserDetails.user_id ? 
