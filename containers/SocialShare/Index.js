...
class Index extends Component {
  ...
  render() {
    ...
    if(this.state.url.indexOf("/watch") > -1 && this.props.pageInfoData.loggedInUserDetails){
        shareUrl = shareUrl+"?ref="+this.props.pageInfoData.loggedInUserDetails.user_id
    }

    const title = this.state.title;
    
    let media = this.state.media
    if(this.props.pageInfoData.livestreamingtype == 0  &&  this.state.media && this.state.media.indexOf(`${this.props.pageInfoData.streamingAppName}/previews`) > 0){
        if(this.props.pageInfoData.liveStreamingCDNURL){
            media = this.props.pageInfoData.liveStreamingCDNURL+this.state.media.replace(`/LiveApp`,'').replace(`/WebRTCAppEE`,'')
        }else{
