...
import Movies from "../Movies/Browse"
...
class Index extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            movies:props.pageInfoData.movies_data ? props.pageInfoData.movies_data : null,
            series:props.pageInfoData.series ? props.pageInfoData.series : null
        }
    }
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        ...
        if(prevState.localUpdate){
            ...
            return {
                ...
                movies:nextProps.pageInfoData.movies_data ? nextProps.pageInfoData.movies_data : null,
                series:nextProps.pageInfoData.series ? nextProps.pageInfoData.series : null,
            }
        } else{
            return null
        }
    ...
    componentDidUpdate(prevProps,prevState){
        if(this.props.pageInfoData.member != prevProps.pageInfoData.member){
...

                                            {
                                                this.state.movies && this.state.movies.results && this.state.movies.results.length > 0 ? 
                                                    <div className={`tab-pane fade${this.state.tabType == "movies" ? " active show" : ""}`} id="movies" role="tabpanel">
                                                        <div className="details-tab-box">
                                                            <Movies {...this.props} globalSearch={true}  user_id={this.state.member.user_id} movies={this.state.movies.results} pagging={this.state.movies.pagging} />
                                                        </div>
                                                    </div>
                                            : null
                                            }
                                            {
                                                this.state.series && this.state.series.results && this.state.series.results.length > 0 ? 
                                                    <div className={`tab-pane fade${this.state.tabType == "series" ? " active show" : ""}`} id="series" role="tabpanel">
                                                        <div className="details-tab-box">
                                                            <Movies {...this.props} typeData={"series"}  globalSearch={true}  user_id={this.state.member.user_id} movies={this.state.series.results} pagging={this.state.series.pagging} />
                                                        </div>
                                                    </div>
                                            : null
                                            }
