...
class Chat extends React.Component{
    ...
    componentDidUpdate(prevProps, prevState) {
        if(this.props.channel != prevProps.channel || this.props.custom_url != prevProps.custom_url || this.props.streamId != prevProps.streamId ){
            ...
            this.props.socket.emit('roomJoin', {room:this.props.channel,streamId:this.props.streamId,custom_url:prevProps.custom_url})
            this.scrollToBottom()
        }
    }
    shouldComponentUpdate(nextProps,nextState){
        if(nextState.tipsData.length != this.state.tipsData ||  nextState.comments != this.state.comments || this.state.comment != nextState.comment || nextState.showTab != this.state.showTab || nextState.channel != this.state.channel || nextState.streamId != this.state.streamId || this.state.hide != nextState.hide){
            return true
        }else{
            return false
        }
    ...
    componentDidMount(){
        this.scrollToBottom()
        //roomJoin
        this.props.socket.emit('roomJoin', {streamId:this.state.streamId,room:this.state.channel,custom_url:this.state.custom_url})
    ...
    hideChat = () => {
        if(this.state.hide){
            this.props.hideShowChat("remove");
        }else{
            this.props.hideShowChat("add");
        }
        this.setState({localUpdate:true,hide:!this.state.hide})
    }
    render(){
        ...
        return(
            <React.Fragment>
                ...
                {
                    this.props.showHideChat ? 
                    <div className="hide-chat" onClick={this.hideChat}>
                        <span className="material-icons-outlined">{
                            this.state.hide ? "keyboard_double_arrow_up" : "keyboard_double_arrow_down"
                        }</span>
                    </div>
                    : null
                }
