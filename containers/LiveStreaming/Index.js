...
class Index extends React.Component{
    constructor(props){
        ...
        this.state = {
            ...
            openAddTip:false,
            scheduled:props.pageInfoData.editItem ? props.pageInfoData.editItem.scheduled : null,
            plans:props.pageInfoData.plans ? props.pageInfoData.plans : [],
            editItem:props.pageInfoData.editItem,
            approved:props.pageInfoData.editItem ? props.pageInfoData.editItem.approve : 1,
        ...
        axios.post(url, formData, config)
            .then(response => {
                if(response.data.error){
                    ...
                    if(this.props.pageInfoData.levelPermissions["livestreaming.scheduled"] == 1 && this.props.pageInfoData.livestreamingtype == 0 && (model["scheduled"] || response.data.approved == 0)){
                        //set custom url
                        ...
                        Router.push(`/create-livestreaming?videoId=${response.data.custom_url}`, `/live-streaming/${response.data.custom_url}`,{ shallow: true })
                        this.setState({localUpdate:true,editItem:response.data.editItem,submitting:false,approved:response.data.approved});
                        return;
                    }
                    this.setState({submitting:false,live:true,videoElem:response.data.editItem,custom_url:response.data.custom_url,video_id:response.data.videoId,image:response.data.image,title:response.data.title,allow_chat:model["enable_chat"],scheduled:model["scheduled"]})
                }
            }).catch(err => {
    ...
    saveTips = (e) => {
        ...
        tips.forEach((item,index) => {
            if(parseFloat(item.amount) > 0){
                if(parseFloat(this.props.pageInfoData.appSettings['livestreaming_commission_value']) > 0 && parseInt(this.props.pageInfoData.appSettings['livestreaming_commission_type']) == 1 && parseFloat(item.amount) <= parseFloat(this.props.pageInfoData.appSettings['livestreaming_commission_value'])){
                    valid = false
                ...
                item.error = this.props.t("Enter amount must be greater than {{price}}.",{price : parseFloat(this.props.pageInfoData.appSettings['livestreaming_commission_value']) > 0 && parseInt(this.props.pageInfoData.appSettings['livestreaming_commission_type']) == 1 ? ReactDOMServer.renderToStaticMarkup(<Currency { ...this.props } {...perprice} />) : 0});
                valid = false;
            }
        })
    ...
    render(){
                        ...
                        <div className="VideoDetails-commentWrap tip_cnt">
                            ...
                            <div className="user_wallet">
                                <div className="row">
                                    <form>
                                        {
                                        parseFloat(this.props.pageInfoData.appSettings['livestreaming_commission_value']) > 0 && parseInt(this.props.pageInfoData.appSettings['livestreaming_commission_type']) == 1 ?
        ...
        if(this.state.errorMessage && this.props.pageInfoData.livestreamingtype == 1){
            return (
        ...
        if(this.props.pageInfoData.livestreamingtype == 0){
            ...
            if(this.state.streamType == "rtmp"){
                ...
                formFields.push(
                    {copyText:true,onClickCopy:this.onClickCopy, key: "stream_url", label: "Server URL",props:{onKeyDown:(e) => {e.preventDefault()}}, value:  "rtmp://"+(this.props.pageInfoData.liveStreamingServerURL.replace("https://","").replace("http://",""))+`/${this.props.pageInfoData.streamingAppName}` },
        ...
        if(this.state.editItem && this.state.editItem.image){
            if(this.state.editItem.image.indexOf("http://") == 0 || this.state.editItem.image.indexOf("https://") == 0){
                ...
                if(this.props.pageInfoData.livestreamingtype == 0 && this.state.editItem.mediaserver_stream_id &&  this.state.editItem.image && this.state.editItem.image.indexOf(`${this.props.pageInfoData.streamingAppName}/previews`) > 0){
                    if(this.props.pageInfoData.liveStreamingCDNURL){
                        imageUrl = this.props.pageInfoData.liveStreamingCDNURL+this.state.editItem.image.replace(`/LiveApp`,'').replace(`/WebRTCAppEE`,'')
                    }else
        ...
        if(this.props.pageInfoData.levelPermissions["livestreaming.scheduled"] == 1 && this.props.pageInfoData.livestreamingtype == 0){
            ...
            let currentTime = dateS.isValid() ? dateS.tz(this.props.pageInfoData.defaultTimezone) : null;
            let minDate = moment(minDateValue).tz(this.props.pageInfoData.defaultTimezone).toDate();
            if(currentTime){
        ...
        if (this.state.streamType) {
            ...
            defaultValues['stream_url'] = "rtmp://"+(this.props.pageInfoData.liveStreamingServerURL ? this.props.pageInfoData.liveStreamingServerURL.replace("https://","").replace("http://","") : "")+`/${this.props.pageInfoData.streamingAppName}`
        }
