...
class Index extends Component {
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        ...
        (nextProps.pageInfoData.password != prevState.password) || nextProps.pageInfoData.adultVideo != prevState.adult) {
           return {
                gateways:false,
                playedVideos:nextProps.pageInfoData.playedVideos ? nextProps.pageInfoData.playedVideos : "",
            ...
            if (this.state.playlistVideos) {
                ...
                videos.forEach( (video, itemIndex) => {
                    ...
                    if(video.is_active_package != 1){
                        ...
                        videos[itemIndex].playerElem = this.props.pageInfoData.appSettings['player_type']
                    }
                })
                let video = this.state.video
                video.playerElem = this.props.pageInfoData.appSettings['player_type']
                this.props.updatePlayerData([], videos, video, deleteMessage, deleteTitle,this.props.pageInfoData.liveStreamingURL)
                ...
                let video = this.state.video
                video.playerElem = this.props.pageInfoData.appSettings['player_type']
                this.props.updatePlayerData([], [], video, deleteMessage, deleteTitle,this.props.pageInfoData.liveStreamingURL)
            }
        ...
        this.props.socket.on('liveStreamStatus', data => {
            ...
            if (this.state.video && this.state.video.owner.idw == id) {
                if(data.action == "liveStreamStarted"){
                    Router.push(`/watch?videoId=${this.state.video.custom_url}`, `/watch/${this.state.video.custom_url}`)
                }
            }
        });
    ...
    goLive = () => {
        Router.push(`/create-livestreaming?videoId=${this.state.video.custom_url}`, `/live-streaming/${this.state.video.custom_url}`)
    }
    setReminder = (e) => {
        if (this.props.pageInfoData && !this.props.pageInfoData.loggedInUserDetails) {
            document.getElementById('loginFormPopup').click();
        ...
        if(this.state.playlistVideos && this.state.playlistVideos.length){
            ...
            if(isAutoplay && this.props.pageInfoData.appSettings['video_autoplay'] == 1 && this.props.pageInfoData.appSettings['enable_iframely'] == 0){
                ...
                if (this.state.relatedVideos && this.state.relatedVideos.length) {
                    ...
                    Router.push(`/watch?videoId=${items[0]['custom_url']}&playedVideos=${playedVideos}`, `/watch/${items[0]['custom_url']}`)
                }
            }
        }
    }
                                                                ...
                                                                <div className="stats">
                                                                    ...
                                                                    <span className="date">
                                                                        ...
                                                                        <div className="subitle">
                                                                            <DateS {...this.props} creation_date={this.state.video.scheduled} initialLanguage={this.props.initialLanguage} format={'MMMM Do YYYY, hh:mm A'} defaultTimezone={this.props.pageInfoData.loggedInUserDetails ? this.props.pageInfoData.loggedInUserDetails.timezone : this.props.pageInfoData.defaultTimezone} />
                                                                        </div>
                                                                        : null
                                                                        }
                                                                    </span>
