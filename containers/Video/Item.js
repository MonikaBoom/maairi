...
class Item extends React.Component {
    ...
    render() {
        ...
        if(this.props.pageInfoData.livestreamingtype == 0 && this.state.video.mediaserver_stream_id &&  !this.state.video.orgImage && this.state.video.is_livestreaming == 1 && parseInt(this.props.pageInfoData.appSettings['antserver_media_hlssupported']) == 1){
            if(this.props.pageInfoData.liveStreamingCDNServerURL){
                videoImage = `${this.props.pageInfoData.liveStreamingCDNServerURL}/${this.props.pageInfoData.streamingAppName}/previews/${this.state.video.mediaserver_stream_id}.png`
            }else
                videoImage = `${this.props.pageInfoData.liveStreamingServerURL}:5443/${this.props.pageInfoData.streamingAppName}/previews/${this.state.video.mediaserver_stream_id}.png`
            ...
            if(this.props.pageInfoData.liveStreamingCDNURL){
                videoImage = `${this.props.pageInfoData.liveStreamingCDNURL}${this.state.video.image.replace(`/LiveApp`,'').replace(`/WebRTCAppEE`,'')}`
            }else
        ...
        return (
            ...
            <React.Fragment>
                {analyticsData}
                {
                    this.props.pageInfoData.appSettings.video_advanced_grid != 1 ? 
                <div className="ptv_videoList_wrap">
                    <div className="videoList_thumb" onMouseEnter={() => this.hoverOn()} onMouseLeave={() => this.hoverOff()} >
                        <Link href="/watch" onClick={this.props.closePopUp} customParam={`videoId=${this.state.video.custom_url}${customParams}`} as={`/watch/${this.state.video.custom_url}${stringId}`}>
                            <a>
                                {
                                    this.state.hover && this.props.pageInfoData.appSettings['player_type'] == "element" && (this.state.video.type == 3 || this.state.video.type == 1) ?
                                        <MediaElementPlayer showControls={false} updateTime={false} muted={true} {...this.props} height="188px" imageSuffix={this.props.pageInfoData.imageSuffix} video={this.state.video} {...this.state.video} />
                                :
                        ...
                        {
                            this.state.video.is_livestreaming && (this.state.video.channel_name || this.state.video.mediaserver_stream_id) && !this.state.video.scheduled ? 
                    ...
                    <div className="videoList_content">
                                ...
                                <ul className="dropdown-menu dropdown-menu-right edit-options">
                                    {
                                        this.props.canEdit ?
                                            this.state.video.scheduled || (this.state.video.approve == 0 && this.state.video.type == 11) ? 
                                                <li>
                                                    <Link href="/create-livestreaming" customParam={`videoId=${this.state.video.custom_url}`} as={`/live-streaming/${this.state.video.custom_url}`}>
                                     ...
                                     {
                                            this.props.deletePlaytistVideo ?
                                            <li>
                                                <a className="addPlaylist addDelete" title={Translate(this.props, "Delete")} href="#" onClick={(e) => this.props.deletePlaytistVideo(e,this.state.video.video_id)}>
                                                    <span className="material-icons" data-icon="delete"></span>
                                                    {Translate(this.props, "Delete")}
                                                </a>
                                            </li>
                        ...
                        {
                            this.state.video.scheduled && (!this.props.pageInfoData.loggedInUserDetails || this.state.video.owner_id != this.props.pageInfoData.loggedInUserDetails.user_id) ? 
                                <div className="scheduled-reminder">
                                    <a onClick={this.setReminder} className="set-reminder" href="#" title={this.props.t("Receive a reminder when this event is scheduled to start.")}>
                        ...
                        <Link href="/watch" onClick={this.props.closePopUp} customParam={`videoId=${this.state.video.custom_url}${customParams}`} as={`/watch/${this.state.video.custom_url}${stringId}`}>
                            <a className="ThumbBox-link" onClick={this.props.closePopUp}>
                                <div className="ThumbBox-coverImg">
                                    <span>  
                                        <Image className={this.state.video.type == 1 ? "yt":""} title={renderToString(<CensorWord {...this.props} text={this.state.video.title} />)} image={videoImage} imageSuffix={this.props.pageInfoData.imageSuffix} />
                                    </span>
                                </div>
                                </a>
                        </Link>
                                {
                                    this.state.video.duration ?
                                        <div className="VdoDuration show-gradient">{this.state.video.duration}</div>
                                        : null
                                }
                                {
                                this.state.video.is_livestreaming && (this.state.video.channel_name || this.state.video.mediaserver_stream_id) && !this.state.video.scheduled ? 
                                ...
                                {
                                    this.props.pageInfoData.appSettings['videos_watchlater'] == 1 ?
                                        <WatchLater className="btnPlayListSave-btn" icon={true} {...this.props} item={this.state.video} id={this.state.video.video_id} />
                                    : null
                                }
                                {
                                    this.props.pageInfoData.appSettings["videos_playlist"] == 1 && this.props.pageInfoData.appSettings["enable_playlist"] == 1 && (!this.props.pageInfoData.levelPermissions || this.props.pageInfoData.levelPermissions['playlist.create'] == 1) && !playlist_id ?
                                        <a className="btnPlayListSave-btn" title={Translate(this.props, "Save to playlist")} onClick={this.playlistOpen} href="#">
                                            <span className="material-icons" data-icon="playlist_add"></span>
                                        </a>
                                        : null
                                }
                                ...
                                <div className="ThumbBox-Title hide-on-expand">
                                    ...
                                    <div className="title ellipsize2Line">
                                        <h4 className="m-0">{<CensorWord {...this.props} text={this.state.video.title} />}</h4>
                                    </div>
                                </div>
                                <div className="ItemDetails">
                                    <div className="d-flex justify-content-between VdoTitle ">
                                        ...
                                        <div className="moreOptions">
                                            ...
                                            {
                                                this.props.canEdit ?
                                                    this.state.video.scheduled || this.props.deletePlaytistVideo || (this.state.video.approve == 0 && this.state.video.type == 11) ? 
                                                    <li>
                                                        <Link href="/create-livestreaming" customParam={`videoId=${this.state.video.custom_url}`} as={`/live-streaming/${this.state.video.custom_url}`}>
                                                            <a className="addPlaylist addEdit"  title={Translate(this.props, "Edit")}>
                                                            <span className="material-icons" data-icon="edit"></span>
                                    ...
                                    <div className="Vdoinfo d-flex flex-column">
                                        {/* {
                                        this.props.pageInfoData.appSettings["videos_username"] == 1  ? 
                                            <UserTitle className="UserName d-inline-flex align-items-center"  onClick={this.props.closePopUp} {...this.props} data={this.state.video} />
                                        : null
                                        } */}
                                        <span className="videoViewDate">
                                        {
