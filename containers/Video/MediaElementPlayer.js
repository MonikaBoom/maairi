...
class MediaElementPlayer extends Component {
		...
		const options = {
			...
			success: function(media) {
                media.addEventListener('timeupdate', function (e) {
                    if(typeof _.props.updateTime == "undefined" && _.player && !_.state.isAdEnabled){
    ...
    render() {
        ...
        if(this.state.video){
            if(this.state.video.type == 3 || this.state.video.type == 'upload'){
            ...
            }else if(this.state.video.code.split(',').length > 1){

                let videos = this.state.video.code.split(',')
                let videoPath = this.props.pageInfoData.liveStreamingServerURL+`:5443/${this.props.pageInfoData.streamingAppName}/streams/`
                ...
                if(videos.length > 1){
                    ...
                    resolutionsVideo.push({
                        src: this.props.pageInfoData.liveStreamingCDNURL ? this.props.pageInfoData.liveStreamingCDNURL+"/streams/"+this.state.video.code : this.props.pageInfoData.liveStreamingServerURL+`:5443/${this.props.pageInfoData.streamingAppName}/streams/`+this.state.video.code,
                        type: 'video/mp4',
                        label: '480p',
                        res: 480
                    })
...
export default connect(null,mapDispatchToProps)(MediaElementPlayer)