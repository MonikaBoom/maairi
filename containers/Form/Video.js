...
class Video extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            videoAmountType: !props.pageInfoData.editItem ? "videotip" : (props.pageInfoData.editItem.mediaserver_stream_id || props.pageInfoData.editItem.channel_name ? "livestreaming" : "videotip")
        }
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        ...
        if(prevState.localUpdate){
            ...
            return {
                ...
                videoAmountType: !nextProps.pageInfoData.editItem ? "videotip" : (nextProps.pageInfoData.editItem.mediaserver_stream_id || nextProps.pageInfoData.editItem.channel_name ? "livestreaming" : "videotip")
            }
        }
    }
    componentDidMount(){
    ...
    onSubmit = model => {
        ...
        if(this.state.sell_videos && this.props.pageInfoData.appSettings['video_commission_type']  == 1 && this.props.pageInfoData.appSettings['video_commission_value'] > 0){
    ...
    uploadMedia = (e) => {
       ...
       var allowedExtensions = ['mp4','mov','webm','mpeg','3gp','avi','flv','ogg','mkv','mk3d','mks','wmv'];
        if (allowedExtensions.indexOf(extension) === -1) 
        {
            ...
            return false;
        }else if( parseInt(this.props.pageInfoData.appSettings['video_upload_limit']) > 0 && e.size > parseInt(this.props.pageInfoData.appSettings['video_upload_limit'])*1000000){
    ...
    saveTips = (e) => {
        ...
        let tips = [...this.state.tips]
        let perprice = {}
        perprice['package'] = { price: this.props.pageInfoData.appSettings[this.state.videoAmountType+'_commission_value'] }
        tips.forEach((item,index) => {
            if(parseFloat(item.amount) > 0){
                if(parseFloat(this.props.pageInfoData.appSettings[this.state.videoAmountType+'_commission_value']) > 0 && parseInt(this.props.pageInfoData.appSettings[this.state.videoAmountType+'_commission_type']) == 1 && parseFloat(item.amount) <= parseFloat(this.props.pageInfoData.appSettings[this.state.videoAmountType+'_commission_value'])){
                    valid = false
                ...
                item.error = this.props.t("Enter amount must be greater than {{price}}.",{price : parseFloat(this.props.pageInfoData.appSettings[this.state.videoAmountType+'_commission_value']) > 0 && parseInt(this.props.pageInfoData.appSettings[this.state.videoAmountType+'_commission_type']) == 1 ? ReactDOMServer.renderToStaticMarkup(<Currency { ...this.props } {...perprice} />) : 0});
                valid = false;
            }
        })
    ...
    render() {
        ...
        if(this.props.pageInfoData.levelPermissions["livestreaming.scheduled"] == 1 && this.props.pageInfoData.livestreamingtype == 0 && this.state.editItem && this.state.editItem.scheduled){
            ...
            let currentTime = dateS.tz(this.props.pageInfoData.defaultTimezone).toDate();
            formFields.push({
                key: "scheduled",
                        ...
                        <div className="VideoDetails-commentWrap tip_cnt">
                            ...
                            <div className="user_wallet">
                                <div className="row">
                                    <form>
                                        {
                                        parseFloat(this.props.pageInfoData.appSettings[this.state.videoAmountType+'_commission_value']) > 0 && parseInt(this.props.pageInfoData.appSettings[this.state.videoAmountType+'_commission_type']) == 1 ?
                ...
                }
                {
                    this.state.success ?
                            !this.state.channel_id ?
                               <React.Fragment>
                                    <Breadcrum {...this.props}  image={this.props.pageData['pageInfo']['banner'] ? this.props.pageData['pageInfo']['banner'] : this.props.pageInfoData['subFolder']+"static/images/breadcumb-bg.jpg"} title={`${this.state.editItem ? "Edit" : "Create"} Video`} />
