...
class Slideshow extends React.Component{
    ...
    render(){
        ...
        let items = this.props.pageInfoData.slideshow.map(elem => {
            ...
            if(this.props.pageInfoData.livestreamingtype == 0 && elem.mediaserver_stream_id &&  !elem.orgImage && elem.is_livestreaming == 1 && parseInt(this.props.pageInfoData.appSettings['antserver_media_hlssupported']) == 1){
                if(this.props.pageInfoData.liveStreamingCDNServerURL){
                    videoImage = `${this.props.pageInfoData.liveStreamingCDNServerURL}/${this.props.pageInfoData.streamingAppName}/previews/${elem.mediaserver_stream_id}.png`
                }else
                    videoImage = `${this.props.pageInfoData.liveStreamingServerURL}:5443/${this.props.pageInfoData.streamingAppName}/previews/${elem.mediaserver_stream_id}.png`
                ...
                if(this.props.pageInfoData.liveStreamingCDNURL){
                    videoImage = `${this.props.pageInfoData.liveStreamingCDNURL}${elem.image.replace(`/LiveApp`,"").replace(`/WebRTCAppEE`,"")}`
                }else
